#!/usr/bin/env python3
"""Import Chrome GraphQL Whop forum-feed captures into parsed archive JSONL."""
...
def combine_content(title: str | None, content: str | None) -> str:
    ...
    if title and content:
        if content.startswith(title):
            return content
        return f"{title}\n\n{content}"
    return title or content or "[empty_or_image_only]"


def build_lookups(existing: dict[str, dict]) -> tuple[
    ...
    by_source_post: dict[tuple[str, str], str] = {}
    for item_id, item in existing.items():
        channel_slug = item.get("channel_slug") or ""
        content_hash = item.get("content_hash") or ""
        local_datetime = item.get("local_datetime") or ""
        exact[(channel_slug, local_datetime, content_hash)] = item_id
        by_hash.setdefault((channel_slug, content_hash), []).append(item_id)
        source_post_id = item.get("source_post_id")
        if source_post_id:
            by_source_post[(channel_slug, source_post_id)] = item_id
        if item.get("has_image"):
            image_only.setdefault((channel_slug, local_datetime[:16]), item_id)
    return exact, image_only, by_hash, by_source_post
...
def choose_message_id(
    ...
    hash_lookup: dict[tuple[str, str], list[str]],
    source_lookup: dict[tuple[str, str], str],
    existing: dict[str, dict],
    post_id: str,
) -> str:
    source_key = (channel_slug, post_id)
    if source_key in source_lookup:
        return source_lookup[source_key]
    exact_key = (channel_slug, local_dt.isoformat(), content_hash)
    if exact_key in exact_lookup:
        return exact_lookup[exact_key]
    hash_key = (channel_slug, content_hash)
    for candidate_id in hash_lookup.get(hash_key, []):
        candidate = existing.get(candidate_id, {})
        if candidate.get("date_precision") in {"date", "approximate"} or candidate.get("time_source") in {
            "zh_date_only",
            "relative_age",
        }:
            return candidate_id
...
def import_capture(path: Path, existing: dict[str, dict]) -> tuple[int, int]:
    ...
    exact_lookup, image_lookup, hash_lookup, source_lookup = build_lookups(existing)
    imported = 0

    for post in posts:
        if post.get("__typename") != "ForumPost":
            continue
        user = post.get("user") or {}
        if user.get("username") != AUTHOR and post.get("userId") != "user_4yeplXgbguTu4":
            continue

        local_dt, et_dt = to_local_iso(post["createdAt"])
        attachments = post.get("attachments") or []
        attachment_ids = [att.get("id") for att in attachments if att.get("id")]
        content = combine_content(post.get("title"), post.get("content"))
        ...
        record = {
            ...
            "source_feed_id": payload.get("feed_id"),
            "forum_comment_count": post.get("commentCount"),
            "forum_pinned": bool(post.get("pinned")),
        }
        imported += 1
        existing[message_id] = record
        exact_lookup[(channel_slug, record["local_datetime"], record["content_hash"])] = message_id
        hash_lookup.setdefault((channel_slug, record["content_hash"]), [])
        if message_id not in hash_lookup[(channel_slug, record["content_hash"])]:
            hash_lookup[(channel_slug, record["content_hash"])].append(message_id)
        source_lookup[(channel_slug, post["id"])] = message_id
        if record["has_image"]:
